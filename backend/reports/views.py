...
def download_bookings_report(request):
    ...
    for booking in bookings:
        data.append([
            ...
            booking.completed_at.strftime(
                '%Y-%m-%d %H:%M') if booking.completed_at else 'N/A',
            booking.service_started_at.strftime(
                '%Y-%m-%d %H:%M') if booking.service_started_at else 'N/A',
...
def download_revenue_report(request):
    ...
    daily_revenue = Booking.objects.filter(
        ...
        total_platform_fees=Sum('payment__platform_fee',
                                filter=Q(status='COMPLETED')),
        stripe_payments=Count('payment', filter=Q(
            payment__payment_method='STRIPE')),
        cod_payments=Count('payment', filter=Q(payment__payment_method='COD')),
        wallet_payments=Count('payment', filter=Q(
            payment__payment_method='WALLET'))
    ).order_by('day')
...
def download_services_report(request):
    ...
    services = ServiceModel.objects.annotate(
        ...
        completed_bookings=Count(
            'booking', filter=Q(booking__status='completed')),
        total_revenue=Sum('booking__payment__final_amount'),
        active_barbers=Count('barber_services', filter=Q(
...
def download_barber_performance_report(request):
    ...
    barbers = User.objects.filter(user_type='barber').annotate(
        ...
        completed_bookings=Count('barber_bookings', filter=Q(
            barber_bookings__status='COMPLETED')),
        cancelled_bookings=Count('barber_bookings', filter=Q(
            barber_bookings__status='CANCELLED')),
        total_earnings=Sum('barber_bookings__total_amount',
                           filter=Q(barber_bookings__status='COMPLETED')),
        avg_rating=Avg('barber_ratings__rating'),
        total_reviews=Count('barber_ratings'),
        active_services=Count('barber_services', filter=Q(
            barber_services__is_active=True))
...
def download_customer_analysis_report(request):
    ...
    customers = User.objects.filter(user_type='customer').annotate(
        ...
        completed_bookings=Count('customer_bookings', filter=Q(
            customer_bookings__status='COMPLETED')),
        cancelled_bookings=Count('customer_bookings', filter=Q(
            customer_bookings__status='CANCELLED')),
        total_spent=Sum('customer_bookings__total_amount',
                        filter=Q(customer_bookings__status='COMPLETED')),
    ...
    for customer in customers:
        ...
        last_booking = customer.customer_bookings.order_by(
            '-created_at').first()
        ...
        data.append([
            ...
            last_booking.created_at.strftime(
                '%Y-%m-%d') if last_booking else 'N/A',
...
def download_financial_summary_report(request):
    ...
    metrics = [
        ...
        ('Total Revenue', f"{float(total_revenue):.2f}",
         'Total amount from completed bookings'),
        ('Platform Fees Collected',
         f"{float(total_platform_fees):.2f}", 'Total platform commission'),
        ('Admin Wallet Balance',
         f"{float(admin_balance):.2f}", 'Current admin wallet balance'),
        ('Customer Wallet Total',
         f"{float(total_customer_wallet):.2f}", 'Total balance in all customer wallets'),
        ('Barber Wallet Total',
         f"{float(total_barber_wallet):.2f}", 'Total balance in all barber wallets'),
        ('Completion Rate %',
         f"{round((completed_bookings/total_bookings*100) if total_bookings > 0 else 0, 2)}", 'Percentage of completed bookings'),
    ]
    ...
    for pm in payment_methods:
        data.append([pm['payment_method'], str(pm['count']),
                    f"Number of {pm['payment_method']} payments"])
